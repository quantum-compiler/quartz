...
# device_get_trajectory = torch.device('cpu')
# device_update = torch.device('cpu')
...
# circ_names = ["all"]
# circ_names = ['barenco_tof_3', 'mod5_4', 'vbe_adder_3', 'gf2^4_mult', 'all']
...
for circ_name in circ_names:
    ...
    if circ_name == 'barenco_tof_3':
        ...
        init_circ = PyGraph.from_qasm(
            context=context, filename=f"../../t_tdg_circs/{circ_name}.qasm"
        )
...
for i_episode in tqdm(range(episodes)):
    ...
    #     qasm_idxs = random.sample(range(1041), 20)
    #     qasm_str = 'OPENQASM 2.0;\ninclude "qelib1.inc";\nqreg q[3];\n'
    #     for idx in qasm_idxs:
    #         with open(f'qasms/{idx}.qasm', 'r') as f:
    #             cnt = 0
    #             for line in f:
    #                 cnt += 1
    #                 if cnt > 3:
    ...
    #     circ = quartz.PyGraph.from_qasm_str(context=context, qasm_str=qasm_str)
    #     sampled_init_circs.append(("all", circ, circ.gate_count))
    ...
    for circ_name in circ_names:
        if circ_name == 'all':
            continue
        circ_gate_cnt = circ_dataset[circ_name]['init_circ'].gate_count
    ...
    for i in range(batch_size - len(circ_names)):
        ...
        #     sampled_init_circs.append(("all", circ, circ.gate_count))
        # else:
        # sampled_circ_name = random.choice(circ_names[:-1])
    ...
    intermediate_circs, trajectory_infos = get_trajectory_beam(
        ppo_agent, context, sampled_init_circs, max_seq_len, invalid_reward, 2
    )
