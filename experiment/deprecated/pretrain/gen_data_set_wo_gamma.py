...
class DataBuffer:
    ...
    def _add_to_reward_map(self, reward, g_hash, node_id, xfer_id, depth):
        if g_hash not in self.reward_map:
            ...
            self.reward_map[g_hash][node_id] = {}
            self.reward_map[g_hash][node_id][xfer_id] = [[reward, depth]]
        elif xfer_id not in self.reward_map[g_hash][node_id]:
            self.reward_map[g_hash][node_id][xfer_id] = [[reward, depth]]
        else:
            self.reward_map[g_hash][node_id][xfer_id].append([reward, depth])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate dataset for pre-training (without gamma).'
    )
