...
def get_circuit_depth(qasm_str):
...
def analyze_circuit(rank, qasm_str, max_depth):
    ...
    initial_graph_depth = get_circuit_depth(initial_graph.to_qasm_str())
    print(f"[Rank {rank}] New circuit {initial_graph_hash} with depth {initial_graph_depth}.")
    ...
    while len(candidate_queue) > 0:
        ...
        for i in range(len(all_nodes)):
            ...
            for xfer in applicable_xfers:
                ...
                new_graph_depth = get_circuit_depth(new_graph.to_qasm_str())
                # if new circuit has fewer gates, then we are done
                if new_graph_depth < initial_graph_depth:
                    final_depth = visited_hash_set[cur_graph_hash] + 1
                    return final_depth, initial_graph_depth - new_graph_depth
...
def worker_proc(rank, circuit_batch, max_depth, folder_suffix):
    result_list = []
    reduced_cnt_list = []
    finished_count = 0
    ...
    # save to file
    with open(f"./tmp{folder_suffix}/{rank}.tmp", 'wb') as handle:
        pickle.dump(obj=result_list, file=handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open(f"./tmp{folder_suffix}/{rank}_reduced_cnt.tmp", 'wb') as handle:
...
def main():
    ...
    # make output dir
    if not os.path.exists(f"./tmp_circuit_depth"):
        os.makedirs(f"./tmp_circuit_depth")
    ...
    for idx in range(num_workers):
        ...
        new_worker = ctx.Process(target=worker_proc,
                                 args=(idx, selected_circuit_list[begin:end],
                                       max_search_depth, "_circuit_depth"))
        new_worker.start()
    ...
    final_results = {}
    for idx in range(num_workers):
        with open(f"./tmp_circuit_depth/{idx}.tmp", 'rb') as handle:
    ...
    print("/*************************************************/")
    with open(f"./final_results_circuit_depth.pickle", 'wb') as handle:
