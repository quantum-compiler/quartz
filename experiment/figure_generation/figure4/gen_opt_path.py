...
def main():
    # get input and make output directory
    parser = argparse.ArgumentParser(description='Generate figure for paper.')
    parser.add_argument('--depth', type=int, required=True)
    args = parser.parse_args()
    assert 1 <= args.depth <= 8
    output_path = f"path_{args.depth}"
    if not os.path.exists(output_path):
        os.makedirs(output_path)

    # read in the circuit
    with open(f"./input/step{args.depth}.qasm", 'r') as handle:
        qasm_str = handle.read()
        final_depth = gen_path(rank=0, qasm_str=qasm_str, max_depth=args.depth)
        assert final_depth == args.depth
