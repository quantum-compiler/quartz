...
class Observer:
    ...
    #             node=graph.get_node_from_id(id=actmp.node))
...
class PPOAgent:
    def __init__(
        ...
        for obs_rank in range(0, num_observers):
            ...
            self.obs_rrefs.append(
                rpc.remote(ob_info,
                           Observer,
                           args=(obs_rank, self.id, batch_inference,
                                 invalid_reward, self.cost_type)))

        self.graph_buffers: List[GraphBuffer] = [
            GraphBuffer(input_graph['name'],
                        input_graph['qasm'], self.cost_type,
                        int(1e6 / len(input_graphs)), self.device)
    ...
    #         temperature = 1.0
    #     else:
    #         temperature = 1 / (math.log(self.hit_rate * (num_nodes - 1) /
    #                                     (1 - self.hit_rate)))
    ...
    #     return ActionTmp(action_node, float(node_values[action_node]),
    #                      xfer_logits)

    # @rpc.functions.async_execution
    # @torch.no_grad()
    # def select_action_batch(self, obs_id: int,
    #                         state_str: str) -> Future[ActionTmp]:
    ...
    #             num_nodes: torch.Tensor = b_state.batch_num_nodes(
    ...
    #             else:
    #                 temperature = 1 / (torch.log(self.hit_rate *
    #                                              (num_nodes - 1) /
    #                                              (1 - self.hit_rate)))

    #             b_softmax_node_values_pad = F.softmax(b_node_values_pad /
    #                                                   temperature.unsqueeze(1),
    #                                                   dim=-1)
    #             b_sampled_nodes = torch.multinomial(b_softmax_node_values_pad,
    #                                                 1).flatten()
    #             """collect embeddings of sampled nodes"""
    #             # (num_graphs, )
    #             node_offsets = torch.zeros(b_sampled_nodes.shape[0],
    ...
    #             xfer_logits: torch.Tensor = self.ac_net.actor(
    #                 sampled_node_embeds).cpu()
    #             # return the xfer dist. to observers who are responsible for sample xfer with masks
    #             actions = [
    #                 ActionTmp(node=int(b_sampled_nodes[i]),
    #                           node_value=float(
    #                               node_values_list[i][b_sampled_nodes[i]]),
    ...
    def output_opt_path(
        self, name: str, best_gc: int,
        exp_seq: List[Tuple[SerializableExperience, quartz.PyGraph,
                            quartz.PyGraph]]
    ) -> str:
    ...
    def collect_data(
        ...
        for obs_rref in self.obs_rrefs:
            ...
            original_graph_qasm_list.append(
                graph_buffer.original_graph.to_qasm_str())
        ...
        for obs_rref, init_qasm, orig_qasm in zip(self.obs_rrefs,
                                                  init_graph_qasm_list,
                                                  original_graph_qasm_list):
        ...
        s_exp_lists: List[List[
            SerializableExperience]] = torch.futures.wait_all(future_exp_lists)
        ...
        for buffer_id, obs_res in zip(init_buffer_ids, s_exp_lists):
            ...
            for s_exp in obs_res:
                ...
                cur_best_cost = get_cost(graph_buffer.best_graph,
                                         self.cost_type)
                if next_graph_cost < cur_best_cost:
                    seq_path = self.output_opt_path(graph_buffer.name,
                                                    next_graph_cost, exp_seq)
        ...
        s_exps: List[SerializableExperience] = list(
            itertools.chain(*s_exp_lists))
    ...
    def sync_best_graph(self) -> None:
        ...
        with open(os.path.join(sync_dir, f'best_info_{self.id}.json'),
                  'w') as f:
    ...
    def select_action_for_self(
        ...
        """compute embeds and readouts"""
        dgl_graphs: List[dgl.DGLGraph] = [g.to_dgl_graph() for g in cur_graphs]
        b_state: dgl.DGLGraph = dgl.batch(dgl_graphs).to(self.device)
        num_nodes: torch.Tensor = b_state.batch_num_nodes().tolist(
        )  # (num_graphs, ) assert each elem > 0
        # (batch_num_nodes, embed_dim)
        b_node_embeds: torch.Tensor
        # (num_graphs, graph_hidden_dim * graph_hidden_layers)
        b_readouts: torch.Tensor
        ...
        for num_node, node_embed, readout_features in zip(
                num_nodes, node_embed_list, b_readout_features):
            readout_features = readout_features.repeat(num_node, 1)
        ...
        masks: torch.Tensor = torch.zeros((num_eps, self.ac_net.action_dim),
                                          dtype=torch.bool)
        masks = cast(torch.BoolTensor, masks)
        ...
        for i, (node_embed,
            ...
            node_logprobs.append(node_logprob.item())
            sampled_node_embeds.append(node_embed[node])
            av_xfers: list[int] = graph.available_xfers_parallel(
                context=qtz.quartz_context,
                node=graph.get_node_from_id(id=node.item()))
            masks[i][av_xfers] = True
        """use Actor to evaluate xfers for sampled nodes"""
        # (num_graphs, action_dim)
        xfer_logits: torch.Tensor = self.ac_net.actor_xfer(
            torch.stack(sampled_node_embeds, dim=0))
        """sample action_xfer with mask"""
        softmax_xfer_logits = masked_softmax(xfer_logits, masks)
        xfer_dists = Categorical(softmax_xfer_logits)
        xfers = xfer_dists.sample()
        xfer_logprobs: torch.Tensor = xfer_dists.log_prob(xfers)
        return dgl_graphs, state_values.tolist(), nodes, xfers.tolist(
        ), node_logprobs, xfer_logprobs.tolist(), masks

    def output_seq(self, name: str, best_cost: int,
                   seq: List[Tuple[quartz.PyGraph, Action, float]]) -> str:
        output_dir = os.path.join(self.output_dir, name,
                                  f'{best_cost}_{self.id}')
        os.makedirs(output_dir)
    ...
    def output_full_opt_seq(
        ...
        output_dir = os.path.join(self.output_dir, name,
                                  f'fullseq_{best_cost}_{self.id}')
        os.makedirs(output_dir)
        graph = best_graph
    ...
    def collect_data_by_self(
        ...
        cur_graphs: List[
            quartz.PyGraph] = init_graph_list.copy()  # shallow copy
        ...
        for i_step in range(max_eps_len_for_all):
            ...
            for inf_batch_start in range(0, num_eps, agent_batch_size):
                mb_dgl_graphs, mb_state_values, mb_action_nodes, mb_action_xfers, mb_node_logps, mb_xfer_logps, mb_av_xfer_masks = self.select_action_for_self(
                    cur_graphs[inf_batch_start:inf_batch_start +
                               agent_batch_size])
                dgl_graphs += mb_dgl_graphs
                state_values += mb_state_values
                ...
                av_xfer_masks = cast(
                    torch.BoolTensor,
                    torch.cat([av_xfer_masks, mb_av_xfer_masks]))
            # end for infer_batch
            """apply actions"""
            for i_eps in range(num_eps):
                ...
                eps_list.state_value.append(state_values[i_eps])
                ...
                eps_list.xfer_mask.append(
                    cast(torch.BoolTensor, av_xfer_masks[i_eps]))
                ...
                if next_graph is not graph and self.output_full_seq:
                    graph_buffer.push_back_all_graphs(next_graph,
                                                      next_graph_cost, graph,
                                                      action)
                if not game_over and not qtz.is_nop(action.xfer) and \
                    ...
                    if next_graph_cost < cur_best_cost:
                        ...
                        for i_glob_step in range(last_eps_end + 1, i_step + 1):
                            ...
                            seq.append((graph_seq[i_eps_step],
                        ...
                        if self.output_full_seq:
                            ...
                            full_seq_path = self.output_full_opt_seq(
                                graph_buffer.all_graphs, graph_buffer.name,
                                next_graph, next_graph_cost)
                            printfl(
