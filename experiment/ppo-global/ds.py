...
class Experience:
    ...
    def __str__(self) -> str:
        s = f'{self.state} (gate_count = {self.state.gate_count}, state_value = {self.state_value}) ' \
            f'{self.next_state} (gate_count = {self.next_state.gate_count}) \n' \
            f'{self.action}  reward = {self.reward}  game_over = {self.game_over}  ' \
            f'xfer_mask = {self.xfer_mask}  xfer_logprob = {self.xfer_logprob}  node_logprob = {self.node_logprob}  info = {self.info}'
        return s
...
class SerializableExperience:
    ...
    def new_empty() -> SerializableExperience:
        return SerializableExperience(
            *[None] * len(fields(SerializableExperience)))  # type:ignore
...
class ExperienceList:
    ...
    def new_empty() -> ExperienceList:
        return ExperienceList(*[[] for _ in range(len(fields(ExperienceList)))
                                ])  # type: ignore

    def shuffle(self) -> None:
        self.state, self.state_value, self.action, self.reward, self.next_state, self.game_over, \
         self.xfer_mask, self.xfer_logprob, self.node_logprob, self.info = shuffle_lists(
            self.state, self.state_value, self.action, self.reward, self.next_state, self.game_over, self.xfer_mask, self.xfer_logprob, self.node_logprob, self.info
        )
    ...
    def get_batch(
        ...
        if start_pos < len(self):
            ...
            exps.state_value = torch.tensor(self.state_value[sc],
                                            device=device,
                                            dtype=torch.float)
            exps.next_state = dgl.batch(self.next_state[sc]).to(device)
            exps.action = torch.stack([a.to_tensor() for a in self.action[sc]
                                       ]).to(device)  # type: ignore
            exps.reward = torch.Tensor(self.reward[sc]).to(device)
            exps.game_over = torch.BoolTensor(self.game_over[sc]).to(
                device)  # type: ignore
            exps.xfer_mask = torch.stack(self.xfer_mask[sc]).to(
                device)  # type: ignore
...
class TrainExpList(ExperienceList):
    target_values: List[float]  # returns
    advantages: List[float]

    @staticmethod
    def new_empty() -> TrainExpList:
        return TrainExpList(*[None] *
                            len(fields(TrainExpList)))  # type: ignore
    ...
    def get_batch(
        ...
        if start_pos < len(self):
            ...
            exps.xfer_logprob = torch.Tensor(self.xfer_logprob[sc]).to(device)
            exps.node_logprob = torch.Tensor(self.node_logprob[sc]).to(device)
            exps.target_values = torch.Tensor(
                self.target_values[sc]).to(device)
...
class BatchedExperience:
    ...
    def new_empty() -> BatchedExperience:
        return BatchedExperience(*[
            None for _ in range(len(fields(BatchedExperience)))
        ])  # type: ignore
    ...
    def __add__(self, other: BatchedExperience) -> BatchedExperience:
        ...
        res.game_over = torch.cat([self.game_over,
                                   other.game_over])  # type: ignore
        res.xfer_mask = torch.cat([self.xfer_mask,
                                   other.xfer_mask])  # type: ignore
    ...
    def __len__(self) -> int:
        return len(self.state.batch_size())
...
class TrainBatchExp(BatchedExperience):
    ...
    def new_empty() -> TrainBatchExp:
        return TrainBatchExp(
            *[None for _ in range(len(fields(TrainBatchExp)))])  # type: ignore
...
class ExperienceListIterator:
    ...
    def __next__(self) -> BatchExpType:
        if self.start_pos < len(self.src):
            ret = self.src.get_batch(self.start_pos, self.batch_size,
                                     self.device)
...
class GraphBuffer:
    ...
    def __init__(
        ...
        self.all_graphs: Dict[quartz.PyGraph, AllGraphDictValue] = {
            self.original_graph:
            AllGraphDictValue(0, self.original_cost, None, Action(0, 0)),
        }
    ...
    def push_back(self, graph: quartz.PyGraph, hash_value: int = None) -> bool:
        ...
        if hash_value not in self.hashset:
            ...
            while len(self.cost_to_graph[gcost]) > int(
                    5e2):  # NOTE: limit the num of graph of each kind
