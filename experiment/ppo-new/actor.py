...
from split_circuit import split_qasm, merge_qasm
...
class PPOAgent:
    ...
    def select_action_batch(self, obs_id: int,
        ...
        with self.lock:  # avoid data race on self.pending_states
            ...
            if self.pending_states == 0:
                ...
                b_node_embeds: torch.Tensor = self.ac_net.graph_embedding(
                    b_state)
                # (batch_num_nodes, )
    ...
    def select_action_for_self(
        ...
        """sample node by softmax with temperature for each graph as a batch"""
        # (num_graphs, max_num_nodes)
        b_node_values_pad = nn.utils.rnn.pad_sequence(node_values_list,
                                                      batch_first=True,
                                                      padding_value=-math.inf)
        # (num_graphs, )
        temperature: torch.Tensor
        if not self.softmax_temp_en:
            ...
            temperature = 1 / (torch.log(self.hit_rate * (num_nodes - 1) /
                                         (1 - self.hit_rate)))
        b_softmax_node_values_pad = F.softmax(b_node_values_pad /
                                              temperature.unsqueeze(1),
                                              dim=-1)
        ...
        av_xfer_masks = torch.zeros_like(
            xfer_logits,
            dtype=torch.bool)  # device is the same with xfer_logits
        ...
        action_node_values: List[float] = b_node_values_pad[
            [i for i in range(num_eps)], action_nodes].tolist()
        return dgl_graphs, action_nodes, action_xfers.tolist(
        ), action_node_values, action_xfer_logps.tolist(), av_xfer_masks.cpu()

    def output_seq(self, name: str, best_gc: int,
    ...
    def collect_data_by_self(
        ...
        circuit_pieces_info_list: list[tuple[str, str, str, str, int]] = []
        ...
        for i_eps in range(num_eps):
            ...
            init_graph = quartz.PyGraph.from_qasm_str(
                context=qtz.quartz_context, qasm_str=qasm_str)
            #####
        ...
        for i_step in range(max_eps_len_for_all):
            ...
            for i_eps in range(num_eps):
                ...
                if i_step > 0:
                    ...
                    if len(last_next_nodes) > 0 and max(
                            last_next_nodes) >= graph.gate_count:
                ...
                if i_step == last_eps_end + 1:
                    ...
                    graph_buffer.rewards.append([])
                    ##### merge circuit
                    graph_qasm_str: str = graph.to_qasm_str()
                    graph_qasm_str = merge_qasm(
                        graph_qasm_str,
                        *(circuit_pieces_info_list[i_eps][1:4]))
                    graph = quartz.PyGraph.from_qasm_str(
                        context=qtz.quartz_context, qasm_str=qasm_str)
                    #####
                ...
                ##### merge circuit
                graph_qasm_str = next_graph.to_qasm_str()
                graph_qasm_str = merge_qasm(
                    graph_qasm_str, *(circuit_pieces_info_list[i_eps][1:4]))
                next_graph_ = quartz.PyGraph.from_qasm_str(
                    context=qtz.quartz_context, qasm_str=graph_qasm_str)
                #####
                graph_buffer.append_costs_from_graph(next_graph_)
                # graph_buffer.append_costs_from_graph(next_graph)
                if not game_over and not qtz.is_nop(action.xfer) and \
                    next_graph_cost <= get_cost(graph_seq[0], self.cost_type):
                    # graph_buffer.push_back(next_graph)
                    ##### cover next_graph_cost
                    graph_buffer.push_back(next_graph_)
                    next_graph_cost = get_cost(
                        next_graph_, self.cost_type)  # cover next_graph_cost
                    # print(next_graph_.gate_count)
                    #####
                    """best graph maintenance"""
                    cur_best_cost = get_cost(graph_buffer.best_graph,
                                             self.cost_type)
                    if next_graph_cost < cur_best_cost:
                        ...
                        seq.append((next_graph_, Action(0, 0), 0))
                        #####
                        ...
                        #####
                        # graph_buffer.best_graph = next_graph
                        graph_buffer.best_graph = next_graph_
                        #####
                    # end if better
                # end if
