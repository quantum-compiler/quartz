...
sleep 5m && \
python ppo.py c=ibm_test c.resume=false c.ddp_port=$(bash gen_port.sh) c.seed=${SEED} c.ckpt_path=${CKPT} c.gnn_num_layers=6 'c.gpus=[0]' 'c.input_graphs=[{ name: "'${CIRC}'", path: "../circs/ibm_circs/'${CIRC}'.qasm"}]' c.num_eps_per_iter=64 c.vmem_perct_limit=${MEM} 2>&1 | tee ftlog/search_ibm_${CIRC}_6l_wopret.log
