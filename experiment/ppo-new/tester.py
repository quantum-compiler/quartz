...
class Tester:
    ...
    def search(
        ...
        while True:
            ...
            latest_info_path = os.path.join(self.output_dir, 'ckpts',
                                            'latest.json')
            ...
            if self.sync_tuning_dir:
                ...
                with open(os.path.join(sync_dir, f'best_info_search.json'),
                          'w') as f:
                    json.dump(best_info, fp=f, indent=2)
        # end while

    @torch.no_grad()
    def random_search(self, circ_name: str,
                      circ: quartz.PyGraph) -> Optional[quartz.PyGraph]:
        ...
        while time.time() - start_time < self.max_search_sec:
            ...
            if False:
                ...
                best_info_path = os.path.join(best_info_dir,
                                              'best_info_0.json')
                if self.sync_tuning_dir and os.path.exists(best_info_path):
                    ...
                    for circ_best_info in best_info:
                        if circ_best_info[
                                'name'] == circ_name and circ_best_info[
                                    'best_cost'] < get_cost(
                                        best_circ, self.cost_type):
                            better_circ = qtz.qasm_to_graph(
                                circ_best_info['qasm'])
                            printfl(
            ...
            if self.sync_tuning_dir and os.path.exists(cir_dir):
                ...
                if len(cir_seq_dirs):
                    ...
                    while better_cost != _better_cost:
                        circ_names = natsorted(os.listdir(better_seq_dir),
                                               reverse=True)
                        if circ_names:
                            better_circ_name = circ_names[
                                0]  # 74_859_0_0_0.qasm
                            better_cost = int(better_circ_name.split('_')[1])
                    better_circ_path = os.path.join(better_seq_dir,
                                                    better_circ_name)
                    while better_cost < get_cost(best_circ, self.cost_type):
                        ...
                        try:
                            ...
                            assert (
                                get_cost(better_circ,
                                         self.cost_type) == better_cost
                            ...
                            printfl(
                                f'Error when reading better circ from tuning: {e}'
                            )
                            continue
            """step forward"""
            ...
            for i, cur_circ in enumerate(cur_circs):
                ...
                elif (get_cost(tmp_best_circ, self.cost_type) == get_cost(
                        best_circ, self.cost_type) == get_cost(
                            cur_circ, self.cost_type)):
    ...
    def random_expand(
        ...
        b_circs: dgl.DGLGraph = dgl.batch(
            [circuit.to_dgl_graph() for circuit in cur_circs]).to(self.device)
        num_nodes: torch.LongTensor = (b_circs.batch_num_nodes()
        ...
        for i_circ, circ in enumerate(cur_circs):
            ...
            soft_valid_pad = torch.zeros((b_node_values_pad.shape[-1], ),
                                         dtype=torch.bool).to(self.device)
            soft_valid = circ_to_soft_valid.get(
                circ,
                torch.ones((circ.gate_count, ),
                           dtype=torch.bool).to(self.device))
            circ_to_soft_valid[circ] = soft_valid
            soft_valid_pad[:circ.gate_count] = soft_valid

            hard_valid_pad = torch.zeros((b_node_values_pad.shape[-1], ),
                                         dtype=torch.bool).to(self.device)
        ...
        # end for
        b_nodes_valid = torch.stack(
            nodes_valid_list)  # (num_graphs, max_num_nodes)
        ...
        # )
        action_xfers: List[int] = torch.argmax(softmax_xfer_logits,
                                               dim=-1,
                                               keepdim=False).tolist()
        next_circs: List[quartz.PyGraph] = []
        for i_circ, circ in enumerate(cur_circs):
            xfer: quartz.PyXfer = qtz.quartz_context.get_xfer_from_id(
                id=action_xfers[i_circ])
            if (xfer.is_nop or circ.gate_count - xfer.src_gate_count +
                    xfer.dst_gate_count > max_cost):
