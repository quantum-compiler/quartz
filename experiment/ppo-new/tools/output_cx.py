...
def init_quartz_context(
    gate_set: List[str] = ['h', 'cx', 't', 'tdg', 'x'],
    ecc_file_path: str = '../ecc_set/t_tdg_ecc.json',
...
init_quartz_context(
    ...
    ecc_file_path='../ecc_set/t_tdg_ecc.json',
    no_increase=False,
    include_nop=True,
)
...
if __name__ == '__main__':
    ...
    for filename in natsorted(os.listdir(dir_path)):
        ...
        if filename.endswith('.qasm'):
            ...
            print(
                f'{graph.cx_count}  {graph.gate_count}  {filename}: {old_gate_count} -> {graph.gate_count}'
            )
