...
class ActorCritic(nn.Module):
    def __init__(self, num_gate_type, graph_embed_size, actor_hidden_size,
        ...
        self.actor_node = nn.Sequential(
            nn.Linear(graph_embed_size, graph_embed_size // 2), nn.ReLU(),
    ...
    def evaluate(self, gs, nodes, xfers):
        ...
        # pool = ParallelPool(os.cpu_count())
        # results = list(pool.map(p, list(range(batched_dgl_gs.batch_size))))
...
class PPO:
    def __init__(self, num_gate_type, context, graph_embed_size,
        ...
        self.optimizer = torch.optim.Adam([{
            ...
            'params':
            self.policy.actor_node.parameters(),
            'lr':
            lr_actor
        }, {
            'params':
            self.policy.actor_xfer.parameters(),
            'lr':
            lr_actor
        }, {
            'params':
    ...
    def update(self):
        ...
        rewards = []
        discounted_reward = 0
        for reward, is_terminal, is_invalid_stop in zip(
                reversed(self.buffer.rewards),
                reversed(self.buffer.is_terminals),
                reversed(self.buffer.is_invalid_stop)):
            if is_terminal and is_invalid_stop:
                discounted_reward = 0
            else:
                pass
        ...
        for _ in range(self.K_epochs):
            ...
            values, node_logprobs, node_entropy, xfer_logprobs, xfer_entropy = self.policy.evaluate(
                self.buffer.graphs, self.buffer.nodes, self.buffer.xfers)

            old_node_logprobs = torch.squeeze(
                torch.stack(self.buffer.node_logprobs,
            ...
            ratios = torch.exp(node_logprobs + xfer_logprobs -
            ...
            loss = actor_loss + 0.5 * critic_loss - 0.01 * (node_entropy +
                                                            xfer_entropy)
...
# TODO: Change this
experiment_name = "rl_ppo_" + "sum_pooling"
...
def get_trajectory(ppo_agent, init_state, max_seq_len, invalid_reward):
    ...
    for t in range(max_seq_len):
        if not done:
            ...
            ppo_agent.buffer.is_invalid_stop.append(
                torch.tensor(invalid_stop, dtype=torch.bool))
