...
class Trajectory:
    def __init__(self, tid: int, init_circ_name: str, init_circ: PyGraph,
                 init_circ_original_cnt: int, init_circ_best_possible_cnt: int,
...
def get_trajectory_batch(ppo_agent: PPO, context: QuartzContext,
        ...
        if len(undone_ts['id']) == 0:
            ...
            for i, id in enumerate(undone_ts['id']):
                trajectory_list[id].apply_action_and_record(
                    context, nodes[i], xfers[i], xfer_logprobs[i], masks[i])
    ...
    for trajectory in trajectory_list:
        ...
        if trajectory.init_circ_name not in trajectory_infos:
            ...
            trajectory_infos[trajectory.init_circ_name]['possible_reward'] = (
                trajectory.init_gate_cnt - trajectory.best_possible_cnt) * 4
            trajectory_infos[
    ...
    for circ in trajectory_infos:
        trajectory_infos[circ]['real_rate'] = trajectory_infos[circ][
            'reward'] / trajectory_infos[circ]['possible_reward']
