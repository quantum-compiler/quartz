...
class ActorCritic(nn.Module):
    ...
    def forward(self, g, context):
        ...
        # critic network generate a v-value for selecting each node
        # gradient required
        node_vs = self.critic(graph_embed).squeeze()
        # node_vs = self.critic(graph_embed)
        # print(node_vs.shape)
        # node_vs, _ = self.critic_attn(graph_embed, graph_embed, node_vs)
        # print(node_vs.shape)
        # print(_)
        # exit(1)
...
def get_trajectory(device, max_seq_len, model, invalid_reward, init_state,
    ...
    for seq_cnt in range(max_seq_len):
        if not done:
            ...
            # Stop with action NOP
                ...
                reward = (graph.gate_count - next_gate_cnt) * 3
                next_value = model.get_value(next_graph, next_nodes)
    ...
    advs = compute_advantages(rewards, values, next_values)
...
def compute_advantages(rewards, values, next_values, gamma=0.9):
