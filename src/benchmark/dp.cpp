...
int main() {
  ...
  std::vector<std::string> circuit_names = {
      "ae",        "dj",     "ghz",   "graphstate", "qft", "qpeexact",
      "su2random", "wstate", "ising", "qsvm",       "vqc", "hhl"};
  // 28-36 total qubits, 28 local qubits
  ...
  for (const auto &circuit : circuit_names) {
    ...
    for (int num_q : num_qubits) {
      std::string file_path = std::string("../../../circuit/MQTBench_") +
      ...
      if (circuit == std::string("hhl")) {
        ...
        file_path = std::string("../../../circuit/NWQBench/") + circuit + "_" +
                    std::to_string(num_q) + ".qasm";
      }
      // requires running test_remove_swap first
      auto seq = CircuitSeq::from_qasm_file(&ctx, file_path);
      ...
      for (int local_q : num_local_qubits) {
        ...
        std::cout << "Schedule for " << circuit << " with " << num_q
                  << " qubits and " << local_q << " local qubits:" << std::endl;
