		...
		for (int i = 0; i < num_outputs; i++, j++) {
			...
			if (edge->output_nodes[i]->output_edges.empty()) {
				...
				for (auto &e : edge->output_nodes[i]->output_edges) {
					auto it =
					    std::find(e->input_nodes.begin(), e->input_nodes.end(),
					              edge->output_nodes[i]);
...
void DAG::generate_hash_values(
	...
	if (kFingerprintInvariantUnderPhaseShift) {
		...
		*main_hash = valhash >> discard_bits;
		other_hash->emplace_back(
		    (valhash + (1 << discard_bits)) >> discard_bits, phase_shift_id);
		return;
	}
...
DAG &DAG::shrink_unused_input_parameters() {
	...
	nodes.erase(nodes.begin() + get_num_qubits() +
	                last_unused_input_param_index,
...
bool DAG::minimal_circuit_representation(std::unique_ptr< DAG > *output_dag,
                                         bool output) const {
	if (output) {
		// |output_dag| cannot be nullptr but its content can (and should) be
		// nullptr.
		assert(output_dag);
			...
			if (output) {
				...
				(*output_dag)
				    ->add_gate(qubit_indices, param_indices,
				               smallest_free_edge->gate, &output_param_index);
