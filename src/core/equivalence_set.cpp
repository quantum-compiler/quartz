...
int EquivalenceSet::remove_unused_qubits_and_input_params(Context *ctx) {
	...
	for (auto &item : classes_) {
		...
		std::vector< bool > input_param_used(rep->get_num_input_parameters(),
		                                     false);
		...
		if (keep_dag_class) {
			...
			// We only need to check the first DAG to see if the class already
			// exists.
			bool first_dag = true;
...
int EquivalenceSet::remove_common_first_or_last_gates(Context *ctx) {
	...
	for (auto &item : classes_) {
		std::unordered_set< DAGHashType > hash_values_to_remove;
		if (item->remove_common_first_or_last_gates(ctx,
