...
void Generator::generate(int num_qubits, int num_input_parameters,
	...
	for (int num_gates = 1; num_gates <= max_num_quantum_gates; num_gates++) {
		...
		if (!verify_equivalences) {
			...
			// This is to make the behavior the same when |verify_equivalences|
			// is true or false.
		...
		if (gate->get_num_qubits() == 0) {
				...
				for (int p1 = 0; p1 < dag->get_num_total_parameters(); p1++) {
					if (restrict_search_space) {
						if (p1 < dag->get_num_input_parameters() &&
						    !used_parameters[p1]) {
							// We should use the new (unused) input parameter
							// with smallest index as the first input if there
			...
			for (int i = 0; i < dag->get_num_qubits(); i++) {
				...
				auto search_parameters =
				    [&](int num_remaining_parameters, auto &
				...
				for (int q1 = 0; q1 < dag->get_num_qubits(); q1++) {
					if (restrict_search_space) {
						if (q1 < dag->get_num_input_parameters() &&
							...
							// smallest index as the first input if there are
					...
					for (int q2 = 0; q2 < dag->get_num_qubits(); q2++) {
						...
						bool ret = dag->add_gate(
						    qubit_indices, parameter_indices, gate, nullptr);
						assert(ret);
						dfs(gate_idx + 1, max_num_gates,
						    max_remaining_param_gates, dag, used_parameters,
						    dataset, restrict_search_space);
		...
		if (verify_equivalences) {
			...
			if (!verifier_.redundant(context, equiv_set, new_dag)) {
				...
				if (new_representatives) {
					// Warning: this is not the new representatives -- only the
					// new DAGs.
			...
			if (ret) {
				...
				// Note: this is the second instance of DAG we create in this
				// function.
