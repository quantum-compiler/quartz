...
namespace quartz {
		...
		for (auto cmd : dst_cmds) {
			...
			for (int i = 0; i < num_params; ++i) {
				...
				// Constant parameters
					...
					graphXfer->paramValues[constant_param.idx] =
					    cmd.constant_params[i];
	...
	GraphXfer::ccz_cx_rz_xfer(Context *ctx) {
		...
		toffoli_rules.second->find_convert_commands(&dst_ctx, GateType::ccz,
		                                            cmd, cmds);
	...
	std::pair< GraphXfer *, GraphXfer * >
	GraphXfer::ccz_cx_u1_xfer(Context *ctx) {
				...
				if (it != mappedInputs.end()) {
					...
					if (newit != newMapInputs.end()) {
						...
						for (it2 = list.begin(); it2 != list.end(); it2++) {
							Edge e = *it2;
							if (e.dstIdx == (int)i) {
								newMapInputs.insert(std::make_pair(
								    in.idx, std::make_pair(e.srcOp, e.srcIdx)));
							}
						}
					}
		...
		for (size_t i = 0; i < srcOp->inputs.size(); i++) {
			...
			if (in.op == NULL) {
				...
				for (it2 = list.begin(); it2 != list.end(); it2++) {
					Edge e = *it2;
					if (e.dstIdx == (int)i) {
						mappedInputs.insert(std::make_pair(
						    in.idx, std::make_pair(e.srcOp, e.srcIdx)));
					}
				}
			}
	...
	Graph *GraphXfer::run_1_time(int depth, Graph *src_graph) {
		if (depth >= (int)srcOps.size()) {
			...
			for (it = src_graph->inEdges.begin();
			     it != src_graph->inEdges.end(); ++it) {
	...
	void GraphXfer::run(int depth, Graph *graph,
	                    ...
	                    int maxNumOps, bool enable_early_stop,
	                    bool &stop_search) {
		if (stop_search)
			return;
		...
		if (depth >= (int)srcOps.size()) {
			...
			std::map< Op, std::set< Edge, EdgeCompare >,
			          OpCompare >::const_iterator it;
			for (it = graph->inEdges.begin(); it != graph->inEdges.end();
			     it++) {
				// printf("can_match(%d)\n", can_match(srcOp, it->first,
				// graph));
				if (can_match(srcOp, it->first, graph) &&
					...
					run(depth + 1, graph, new_candidates, hashmap, threshold,
					    maxNumOps, enable_early_stop, stop_search);
	...
	Graph *GraphXfer::create_new_graph(const Graph *graph) const {
		...
		for (opIt = graph->inEdges.begin(); opIt != graph->inEdges.end();
		     opIt++)
				...
				for (it = list.begin(); it != list.end(); it++)
					if (mappedOps.find(it->srcOp) != mappedOps.end()) {
						...
						if (dstTen.op == NULL) {
							...
							newGraph->add_edge(srcEdge.first, it->dstOp,
							                   srcEdge.second, it->dstIdx);
						}
						else {
		...
		for (dstIt = dstOps.begin(); dstIt != dstOps.end(); dstIt++) {
			...
			for (size_t i = 0; i < dstOp->inputs.size(); i++)
				if (dstOp->inputs[i].op == NULL) {
					...
					if (paramValues.find(dstOp->inputs[i].idx) !=
					    paramValues.end()) {
						// New constant parameters
						Op input_constant_param_op(
						    newGraph->get_next_special_op_guid(),
					...
					std::multimap< int, std::pair< Op, int > >::const_iterator
					    it = mappedInputs.find(dstOp->inputs[i].idx);
