...
namespace quartz {
	...
	bool equal_to_2k_pi(double d) {
		d = std::abs(d);
		int m = d / (2 * PI);
		if (std::abs(d - m * 2 * PI) > eps &&
		    std::abs(d - (m + 1) * 2 * PI) > eps)
			return false;
		return true;
	}
	...
	Graph::Graph(Context *ctx, const DAG &dag)
		...
		for (auto &node : dag.nodes) {
			...
			if (node->type == DAGNode::input_qubit) {
				...
				assert(node->input_edges.size() ==
				       1); // A node can have at most 1 input
				auto input_edge = node->input_edges[0];
				bool found = false;
				for (srcIdx = 0; srcIdx < input_edge->output_nodes.size();
				     ++srcIdx) {
			...
			for (auto output_edge : node->output_edges) {
				int dstIdx;
				bool found = false;
				for (dstIdx = 0; dstIdx < output_edge->input_nodes.size();
				     ++dstIdx) {
	...
	void Graph::add_edge(const Op &srcOp, const Op &dstOp, int srcIdx,
	                     int dstIdx) {
	...
	Graph *Graph::context_shift(Context *src_ctx, Context *dst_ctx,
		...
		auto dst_gate_set =
		    std::set< GateType >(dst_ctx->get_supported_gates().begin(),
					...
					for (auto out_edge : output_edges) {
						if (in_edge.dstIdx == out_edge.srcIdx) {
							if (in_edge.dstIdx < num_qubits) {
								add_edge(in_edge.srcOp, out_edge.dstOp,
								         in_edge.srcIdx, out_edge.dstIdx);
							}
						}
					}
				}
		...
		while (!op_queue.empty()) {
			...
			if (op.ptr->is_parameter_gate()) {
				...
				for (auto it = list.begin(); it != list.end(); ++it) {
					...
					if (constant_param_values.find(src_op) ==
				...
				for (auto in_edge : input_edges) {
					if (in_edge.dstIdx >= num_qubits &&
						...
						if (constant_param_values.find(in_edge.srcOp) ==
							...
							if (!equal_to_2k_pi(
							        constant_param_values[in_edge.srcOp])) {
	...
	void Graph::expand(Pos pos, bool left, GateType target_rotation,
		...
		while (true) {
			...
			if (pos.op.ptr->tp == GateType::cx) {
				...
				if (anchor_point.find(
				        pos_to_qubits[Pos(pos.op, pos.idx ^ 1)]) ==
				    anchor_point.end()) {
	...
	void Graph::rotation_merging(GateType target_rotation) {
		...
		while (!todos.empty()) {
			...
			if (op.ptr->tp == GateType::cx) {
				...
				bitmasks[Pos(op, 1)] =
				    bitmasks[pos_list[0]] ^ bitmasks[pos_list[1]];
		...
		while (!todo_cx.empty()) {
			...
			while (!todo_qubits.empty()) {
				...
				todo_qubits.pop();
				expand(anchor_point[qid], true, target_rotation, covered,
				       anchor_point, pos_to_qubits, todo_qubits); // expand left
				expand(anchor_point[qid], false, target_rotation, covered,
				       anchor_point, pos_to_qubits,
			...
			std::unordered_map<
			    int, std::unordered_map< uint64_t,
			                             std::unordered_set< Pos, PosHash > > >
			    qubit_2_bm_2_pos;
			for (const auto &pos : covered) {
				...
				if (pos.op.ptr->tp == target_rotation) {
					...
					if (qubit_2_bm_2_pos.find(qubit_idx) ==
					    qubit_2_bm_2_pos.end()) {
				...
				for (const auto &it_1 : it_0.second) {
					...
					for (auto pos : pos_set) {
						if (is_first) {
							...
							std::cout << "merging op "
							          << gate_type_name(first.op.ptr->tp) << "("
							          << first.op.guid << ")"
							          << " and "
							          << gate_type_name(pos.op.ptr->tp) << "("
					...
					if (all_param_is_0) {
						...
						remove_node(op);
						std::cout << "eliminating op "
						          << gate_type_name(op.ptr->tp) << "("
						          << op.guid << ")" << std::endl;
					}
				}
			}
		...
		while (!todos.empty()) {
			...
			if (op.ptr->tp != GateType::input_qubit &&
				...
				for (auto edge : in_edges) {
					if (edge.dstIdx < num_qubits) {
						assert(
						    pos_to_qubits.find(Pos(edge.srcOp, edge.srcIdx)) !=
						    pos_to_qubits.end());
						q_idx[edge.dstIdx] =
	...
	void Graph::draw_circuit(const std::string &src_file_name,
		...
		system(("python python/draw_graph.py " + src_file_name + " " +
		        save_filename)
		           .c_str());
	}

	Graph *Graph::optimize(float alpha, int budget, bool print_subst,
	                       Context *ctx, const std::string &equiv_file_name,
		...
		std::cout
		    << std::dec << eqs.num_equivalence_classes()
		...
		if (use_simulated_annealing) {
			...
			int num_iteration = 0;
			std::cout << "Begin simulated annealing with " << xfers.size()
			          << " xfers." << std::endl;
			for (double T = kSABeginTemp; T > kSAEndTemp;
			     T *= kSACoolingFactor) {
				num_iteration++;
				hashmap.clear();
				...
				for (auto &candidate : sa_candidates) {
					...
					for (auto &xfer : xfers) {
						xfer->run(0, candidate.second, current_new_candidates,
						          hashmap, bestCost * alpha, 2 * maxNumOps,
						          enable_early_stop, stop_search);
					}
					num_possible_new_candidates +=
					    current_new_candidates.size();
					for (auto &new_candidate : current_new_candidates) {
						...
						if (new_cost < current_cost ||
							...
							new_candidates.emplace_back(new_cost,
							                            new_candidate);
						}
						else {
					...
					if (!always_delete_original_circuit &&
					    ...
					    hashmap.find(candidate.second->hash()) ==
					        hashmap.end()) {
				...
				if (new_candidates.empty()) {
					std::cout << "No new candidates. Early stopping."
					          << std::endl;
					break;
				}
				...
				if (new_candidates.size() > kNumKeepGraph) {
					...
					std::partial_sort(new_candidates.begin(),
					                  ...
					                  new_candidates.end());
					for (int i = kNumKeepGraph; i < (int)new_candidates.size();
					     i++) {
				...
				std::cout << "Iteration " << num_iteration
				          ...
				          << num_new_candidates - num_candidates_kept
				          << " out of " << num_possible_new_candidates
			...
			while (!candidates.empty()) {
				...
				bool stop_search = false;
				for (auto &xfer : xfers) {
					xfer->run(0, subGraph, new_candidates, hashmap,
