...
void DistributedSimulator::init_devices() {
  ...
  IndexLauncher initLauncher(CUDA_INIT_TASK_ID,
                             task_is,
...
/*static*/
void DistributedSimulator::top_level_task(Task const *task,
  ...
  auto seq = quartz::CircuitSeq::from_qasm_file(
      &ctx, std::string("/home/ubuntu/quartz/circuit/MQTBench_") +
                ...
                "_indep_qiskit_" + std::to_string(config.num_all_qubits) + ".qasm");
  sim::qcircuit::Circuit<double> circuit(config.num_all_qubits, config.num_local_qubits);
...
bool DistributedSimulator::create_regions()
{
  ...
  Point<1> ext_hi;
  ext_hi[i] = (state_vec_rect.hi[0] - state_vector_rect.lo[0] + total_gpus) / total_gpus - 1;
  ...
  assert((1 << (size_t)config.num_all_qubits) % (state_vec_rect.hi[0] + 1) == 0);
  size_t num_state_vectors = (1 << (size_t)config.num_all_qubits)
                           / (state_vec_rect.hi[0] + 1);
...
bool DistributedSimulator::apply_gates(const std::vector<Gate<DT> >& gates) {
  for (size_t i = 0; i < cpu_state_vectors.size(); i++) {
    ...
    std::pair<LogicalRegion, LogicalPartition> gpu_sv = gpu_state_vectors[i % gpu_state_vectors.size()];
    // Step 1: move a state vector from DRAM to GPU memory
    {
      IndexCopyLauncher launcher(parallel_is);
      launcher.add_copy_requirements(
          RegionRequirement(cpu_sv.second,
                            0/*projection ID*/,
                            READ_ONLY,
                            EXCLUSIVE,
                            cpu_sv.first,
                            MAP_TO_ZC_MEMORY),
          RegionRequirement(gpu_sv.second,
                            0/*projection ID*/,
                            WRITE_DISCARD,
                            EXCLUSIVE,
                            gpu_sv.first)
    ...
    // Step 2: launch all gate kernels
    {
      ...
      IndexLauncher launcher(GATE_COMP_TASK_ID,
                             parallel_is,
                             TaskArgument(&info, sizeof(GateInfo)),
                             argmap,
                             Predicate::TRUE_PRED,
                             false /*must*/,
                             0 /*mapper_id*/);
      launcher.add_region_requirement(RegionRequirement(gpu_sv.second,
                                                        0 /*projection ID*/,
                                                        READ_WRITE,
                                                        EXCLUSIVE,
                                                        gpu_sv.first));
    ...
    // Step 3: move a state vector from GPU memory to DRAM
    {
      IndexCopyLauncher launcher(parallel_is);
      launcher.add_copy_requirements(
          RegionRequirement(gpu_state_vectors[i % gpu_state_vectors.size()].second,
                            0/*projection ID*/,
                            WRITE_DISCARD,
                            EXCLUSIVE,
                            gpu_state_vectors[i % gpu_state_vectors.size()].first),
          RegionRequirement(cpu_state_vectors[i].second,
                            0/*projection ID*/,
                            READ_ONLY,
                            EXCLUSIVE,
                            cpu_state_vectors[i].first,
                            MAP_TO_ZC_MEMORY)
      );
      launcher.add_src_field(0, FT_DATA);
      launcher.add_dst_field(0, FT_DATA);
...
int main(int argc, char **argv) {
  ...
  Runtime::set_top_level_task_id(TOP_LEVEL_TASK_ID);
  {
    ...
    Runtime::preregister_task_variant<DistributedSimulator::top_level_task>(registrar, "top_level");
  }
  {
    TaskVariantRegistrar registrar(CUDA_INIT_TASK_ID,
                                   "CUDA Init");
  ...
  {
    TaskVariantRegistrar registrar(GATE_COMP_TASK_ID,
                                   "Gate Compute");
    registrar.add_constraint(ProcessorConstraint(Processor::TOC_PROC));
    registrar.set_leaf();
