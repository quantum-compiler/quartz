...
int main() {
    ...
    // set a mapping and test its cost
    std::vector<int> logical2physical{0, 1, 2, 3, 4};
    graph.set_physical_mapping(logical2physical);
    MappingStatus succeeded2 = graph.check_mapping_correctness();
    if (succeeded2 != quartz::MappingStatus::VALID) {
        std::cout << "Mapping test failed!" << endl;
    }
    double target_cost = graph.circuit_implementation_cost(device);
    cout << "Target implementation cost is " << target_cost << endl;
};
