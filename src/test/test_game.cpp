...
int main() {
    ...
    cout << "This is test game on " << circuit_file_name << ".\n";
    ...
    graph.init_physical_mapping(InitialMappingType::SABRE, device,
                                3, true, 0.8);
    graph.check_mapping_correctness();
    double sabre_implementation_cost = graph.circuit_implementation_cost(device);
    double sabre_real_cost = execution_cost(sabre_swap(graph, device, true, 0.8));
