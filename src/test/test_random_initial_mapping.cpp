...
int main() {
    // tof_3_after_heavy.qasm, gf2^E5_mult_before.qasm
    ...
    graph.init_physical_mapping(InitialMappingType::RANDOM, device, -1, false, -1);
    ...
    for (const auto& input_qubit_mapping : graph.qubit_mapping_table) {
        ...
        std::cout << "Physical idx: " << input_qubit_mapping.second.second << std::endl << std::endl;
    }
    if (graph.check_mapping_correctness() == MappingStatus::VALID)
        std::cout << "Test has passed\n";
    else std::cout << "Error in mapping!\n";
};
