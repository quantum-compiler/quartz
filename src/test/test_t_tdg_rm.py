        ...
        for xfer in context.get_xfers():
            for node in all_nodes:
                ...
                if print_message and invoke_cnt % 100_000_000 == 0:
                    print(
                        f"[{circ_name} (rotation merging)] best t count: {best_t_cnt}, candidate count: {len(candidate)}, API invoke time: {invoke_cnt}, time cost: {t - start:.3f}s"
                    )
...
if __name__ == '__main__':
    ...
    circ = quartz.PyGraph.from_qasm(
        context=context, filename=f"../../circuit/t_tdg_rm_circs/{circ_name}.qasm"
    )
