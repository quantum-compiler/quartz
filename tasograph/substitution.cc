...
OpX::OpX(::GateType _type)
: type(_type)
{
}

void OpX::add_input(const TensorX& input)
{
  inputs.push_back(input);
}

void OpX::add_output(const TensorX& output)
{
  outputs.push_back(output);
}

GraphXfer::GraphXfer(::Context* _context)
...
: context(_context), tensorId(10)
{
  ...
  assert(src_graph->get_num_input_parameters() == dst_graph->get_num_input_parameters());
  ...
  for (int i = 0; i < src_graph->get_num_qubits(); i++) {
    ...
    map_output(src_to_tx[src_graph->outputs[i]], dst_to_tx[dst_graph->outputs[i]]);
  }
}
...
{
  if (srcOp->type != op.ptr->tp) return false;
