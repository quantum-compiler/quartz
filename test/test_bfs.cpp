...
int main() {
  Context ctx(
      {GateType::h, GateType::x, GateType::cx, GateType::rz, GateType::add});
  Generator gen(&ctx);

  const int num_qubits = 3;
  const int num_input_parameters = 3;
  const int max_num_quantum_gates = 5;
  ...
  if (run_bfs_verified) {
    ...
    equiv_set.simplify(&ctx);
    equiv_set.save_json("voqc_ecc.json");
    end = std::chrono::steady_clock::now();
    std::cout
